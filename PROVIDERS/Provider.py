...
def isActive(provider_code):
    if provider_code < len(Providers):
        return Providers[provider_code]["active"]
    raise Exception("isActive() index out of range")

def getProviderByProviderCode(provider_code):
    if not provider_code < len(Providers):
        raise Exception("getProviderNameByCode() index out of range")

    provider_name = getProviderNameByCode(provider_code)
    return getProviderByProviderName(provider_name)

def getProviderByProviderName(provider_name):
...
def getProviderNameByCode(provider_code):
    if not provider_code < len(Providers):
        raise Exception("getProviderNameByCode() index out of range")
    return Providers[provider_code]["name"]

def getProviderCodeByName(provider_name):
    for provider in Providers:
        if provider["name"] == provider_name:
            return provider["code"]
    return None

def getArticleBaseURLbyProviderName(provider_name, article_name):
    provider = getProviderByProviderName(provider_name)
    return provider.getProductUrl(article_name)

def cloneProvider(provider):
    return getProviderByProviderName(provider.getName())
...
class Provider:
    ...
    def __init__(self):
        print(f"Provider --init(): {self} ")

    def __del__(self):
        print(f"Provider --del(): {self} ")

    def __exit__(self):
        print(f"Provider: --exit: {self} ")
